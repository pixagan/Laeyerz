...
"""
NodePicker module for node selection
...
def node_picker(node_type, node_name, params):

    if node_type == "Test_Node":
        newNode = TestNode(node_name, params)
        return newNode
        #return create_test_node(node_name)

    elif node_type == "Prompt":
        newNode = PromptNode(node_name, params)
        #return create_test_node(node_name)
        return newNode

    elif node_type == "OpenAI":
        newNode = OpenAILLMNode(node_name, params)
        return newNode
        #return create_openai_node(node_name)

    elif node_type == "Chroma":
        newNode = ChromaDBNode(node_name, params)
        return newNode

    elif node_type == "Faiss":
        return create_faiss_node(node_name, params)
        ...
        return create_sqlite_node(node_name, params)


    elif node_type == "FileInput":
        newNode = FileInputNode(node_name, params)
        return newNode

    elif node_type == "TextInput":
        newNode = TextInputNode(node_name, params)
        return newNode
        

    elif node_type == "Custom":
        customNode = Node(node_type, node_name, params)
